# LE Connection Oriented Channel (COC) (Server/Client) Sample application

## Features demonstrated
- AIROC&#8482; LE L2CAP APIs for Connection Oriented Channels
...
Optionally, define flags in the application makefile to operate in standalone mode without interaction with ClientControl.
See STANDALONE_MODE flags in the makefile to define central or peripheral operation mode.
In standalone mode,
	Central Device:
	- Registers fixed L2CAP PSM
	- Start LE scan and looks up for LE_COC device name.
	- Once Peripheral device is found, the Central connects automatically, updates to use 2M PHY, and start sending data
	- Once test is complete, display send data details with throughput
	Peripheral Device:
	- Registers fixed L2CAP PSM
	- Start LE advertisements
	- Wait for a connection from the peer application
	- On connection, wait for data from the peer
	- Once test is complete, displays receive data details with throughput
